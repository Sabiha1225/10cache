...
from deepspeed.runtime.utils import prefetchtable, memory_manager
...
class DeepSpeedEngine(Module):
    ...
    def start_training_time(self):
        self.timers.start_training_time()
    
    def end_training_time(self):
        self.timers.end_training_time()

    def set_prefetch_table_warmup(self):
        prefetchtable.set_warmup(False)
        
    def preallocate_memory(self):
        self.optimizer.preallocate_memory()
    ...
    def _configure_distributed_model(self, model):
        ...
        if not (self.dont_change_device or is_zero_init_model):
            ...
            #     file.write(f"/deepspeed/runtime/zero/engine.py#LN479 inside def_configure_distributed_model gpu_available_memory before model_to {memory_manager.get_gpu_available_memory_nvidia_smi()} cpu_available_memory before {memory_manager.get_cpu_available_memory()}  \n")
            self.module.to(self.device)
            # with open("/home/sabiha/deepspeed_example/deepspeed_gpu_cpu_tensor_count.txt", 'a') as file:
            #     file.write(f"/deepspeed/runtime/zero/engine.py#LN479 inside def_configure_distributed_model gpu_available_memory after model_to {memory_manager.get_gpu_available_memory_nvidia_smi()} cpu_available_memory before {memory_manager.get_cpu_available_memory()} \n")
