...
from deepspeed.runtime.utils import prefetchtable, memory_manager, active_tensor_window, GPU_MEMORY_CACHE, CPU_MEMORY_CACHE, params_id_list, gpu_threshold
from deepspeed.runtime.utils import additional_buffer_factor, CPU_TENSOR_SIZE_COUNT, GPU_TENSOR_SIZE_COUNT, swap_space_gpu, swap_space_cpu, nvme_buffer_space, cpu_threshold, min_buffer_count
from deepspeed.runtime.utils import GPU_RANK, CPU_RANK, NVME_RANK, gpu_grad_threshold, optimizer_states_buffer_factor, OPTIMIZER_STATES_CACHE, OPTIMIZER_STATES_TENSOR_SIZE_COUNT, fp32_threshold
from deepspeed.runtime.utils import cpu_additional_buffer_factor, offloadcountinfo
import time
import csv
import ctypes
import math
...
class DeepSpeedZeroOptimizer_Stage3(ZeROOptimizer):
    ...
    def __init__(
        ...
        self.param_count_gpu = param_count_gpu
        print(f"stage3.py self.param_count_gpu {self.param_count_gpu}")
        ...
        if self.swap_optimizer:
            ...
            self.offload_optimizer_config = offload_optimizer_config
        ...
        self.largest_param_numel = largest_partitioned_param_numel
    ...
    def destroy(self):
        ...
        del prefetchtable
        del memory_manager
        del GPU_MEMORY_BUFFER
        del CPU_MEMORY_BUFFER
        del GPU_MEMORY_CACHE
        del CPU_MEMORY_CACHE
        del OPTIMIZER_STATES_CACHE
        del GPU_TENSOR_SIZE_COUNT
        del CPU_TENSOR_SIZE_COUNT
        del OPTIMIZER_STATES_TENSOR_SIZE_COUNT
        del active_tensor_window
        del params_id_list
        del offloadcountinfo

    
    def allocate_cache_memory(self):
        for tensor_size, c in self._get_param_coordinator(training=True).tensor_size_dict.items():
            if c == 1 or c == 2:
                GPU_MEMORY_CACHE.add_free_bufferes(tensor_size, int(tensor_size), c, self.dtype, get_accelerator().current_device_name())
                CPU_MEMORY_CACHE.add_free_bufferes(tensor_size, int(tensor_size), c, self.dtype, 'cpu')
            else:
                GPU_MEMORY_CACHE.add_free_bufferes(tensor_size, int(tensor_size), gpu_threshold, self.dtype, get_accelerator().current_device_name())
    ...
    def tensor_sized_ratio_correction(self, buffer_size_count_mapping):
        ...
        for key in buffer_size_count_mapping:
            if buffer_size_count_mapping[key] != 1 and buffer_size_count_mapping[key] != 2 and buffer_size_count_mapping[key] < min_buffer_count :
                ...
                elif int(max_key) % int(key) == 0 and buffer_size_count_mapping[max_key] > 100:
                    key_remainder = (min_buffer_count - buffer_size_count_mapping[key])
                    per_count = int(int(max_key) / int(key))
                    deduct = int(math.ceil(key_remainder/per_count))
                    if buffer_size_count_mapping[max_key] - deduct > 100:
                        buffer_size_count_mapping[max_key] -= deduct
                        buffer_size_count_mapping[key] += int(key_remainder)

                    else:
                        remainder = buffer_size_count_mapping[max_key] - 100
                        buffer_size_count_mapping[key] += remainder * per_count
                        buffer_size_count_mapping[max_key] -= remainder

        return buffer_size_count_mapping

    def tensor_sized_ratio_correction_fp32(self, buffer_size_count_mapping):
        max_key = max(buffer_size_count_mapping, key=buffer_size_count_mapping.get)
        min_key = min(buffer_size_count_mapping, key=buffer_size_count_mapping.get)
        sorted_keys = sorted(buffer_size_count_mapping, key=buffer_size_count_mapping.get, reverse=True)

        for key in buffer_size_count_mapping:
            if buffer_size_count_mapping[key] != 1 and buffer_size_count_mapping[key] != 2 and buffer_size_count_mapping[key] < 15 :
                if int(key) % int(max_key) == 0:
                    per_count = int(int(key) / int(max_key))
                    buffer_size_count_mapping[key] += 2
                    buffer_size_count_mapping[max_key] -= (2 * per_count)
                if int(sorted_keys[1]) % int(key) == 0:
                    per_count = int(int(sorted_keys[1]) / int(key))
                    if per_count > 5:
    ...
    def calculate_tensor_size_distribution(self):
        total_tensors = 0
        self.param_tensor_count_total = 0
        self.tensor_size_distribution = {}
        for tensor_size, c in self._get_param_coordinator(training=True).tensor_size_dict.items():
    ...
    def calculate_tensor_size_distribution_optimizer_states(self):
        ...
        self.tensor_size_distribution_optimizer_states = {}
        ...
        for tensor_size, c in self._get_param_coordinator(training=True).tensor_size_dict.items():
            ...
            count = int(new_tensor_size) * c
            total_tensors += count
            self.tensor_size_distribution_optimizer_states[new_tensor_size] = count * 1.0
    ...
    def clear_cpu_memory(self):
        ...
        #defragmented pinned memory
        del self.param_groups_fp16_flat_cpu_memory

        #a single 32-bit partition of the parallel partitioned parameters
        #that this process will update
        del self.fp32_partitioned_groups_flat
        del self.next_swappable_fp32_partitioned_groups
        ...
        #clear cpu buffer for NVMe memory
        self.fp16_groups[0][0].nvme_swapper.clear_buffer()
        #self.optimizer_swapper = None
        gc.collect()

    def calculate_active_tensor_window_gpu(self):
    ...
    def preallocate_memory(self):
        self.persistent_parameters.clear()
        ...
        for i in range(gpu_threshold):
            ...
            tensor_type = prefetch_info.tensor_type
            tensor_element = prefetch_info.tensor_element
            layer_id = prefetch_info.layer_id
            fwd = prefetch_info.fwd
            loc = prefetch_info.loc
            ...
            if(prefetch_info.tensor_type=="parameter_16" and prefetch_info.fwd=="fwd"):
                ...
                prefetch_info.loc = OffloadDeviceEnum.gpu
                active_tensor_window.append(tensor_id)
                ...
                if param.ds_tensor.status == PartitionedParamStatus.NOT_AVAILABLE:
                    ...
                    param.ds_tensor.data = GPU_MEMORY_CACHE.add(param.ds_tensor, buffer_id)
        ...
        for i in range(gpu_threshold, gpu_threshold + cpu_threshold):
            ...
            if(prefetch_info.fwd=="bwd"):
                ...
                if param.ds_tensor.status == PartitionedParamStatus.NOT_AVAILABLE:
                    ...
                    param.ds_tensor.data = CPU_MEMORY_CACHE.add(param.ds_tensor, buffer_id)
                ...
                index = prefetchtable.get_tensor_id_to_prefetch_list_id(param.ds_id, "fwd")
                prefetchtable.get_prefetch_information_list_item(index).current_location = OffloadDeviceEnum.cpu
        ...
        if gpu_threshold + cpu_threshold < self.param_tensor_count_total:
            ...
            for i in range(gpu_threshold + cpu_threshold, len(prefetch_information_list)):
                prefetch_info = prefetch_information_list[i]
                tensor_id = prefetch_info.tensor_id
                tensor_type = prefetch_info.tensor_type
                tensor_element = prefetch_info.tensor_element
                ...
                if(prefetch_info.fwd=="bwd"):
                    ...
                    prefetchtable.PARAMS_IN_NVME = True
                    if param.ds_tensor.status == PartitionedParamStatus.NOT_AVAILABLE:
                        param.ds_tensor.final_location = OffloadDeviceEnum.nvme
                        #continue
                    else:
                        buffer = param.nvme_swapper.get_buffer(param, tensor_element)
                        buffer.data.copy_(param.ds_tensor.data)
                        param.ds_tensor.data = buffer.data
                        param.ds_tensor.final_location = OffloadDeviceEnum.nvme
                    ...
                    index = prefetchtable.get_tensor_id_to_prefetch_list_id(param.ds_id, "fwd")
                    prefetchtable.get_prefetch_information_list_item(index).current_location = OffloadDeviceEnum.nvme
        ...
        for i, param_group in enumerate(self.trainable_param_groups):
            for param in param_group["params"]:
                self.sub_group_to_group_id_new[param.ds_id] = i
    ...
    def move_param_32_grad_to_nvme_optimized(self):
        ...
        gradient_dtype = torch.float32
        self.fp32_partitioned_groups_flat_new = []
        self.fp32_active_tensor_window = []
        swappable_fp32_tensors = []
        swappable_fp16_src_tensors = []
        self.fp32_param_location = {}
        ...
        for i, param in enumerate(all_params):
            ...
            if not prefetchtable.FP32_IN_NVME:
                ...
                buffer_id = OPTIMIZER_STATES_CACHE.get_free_buffer_id(num_elements)
                OPTIMIZER_STATES_CACHE.add_param_id_to_buffer_id(param.ds_id, buffer_id)
                OPTIMIZER_STATES_CACHE.add_occupied_buffer_id(param.ds_id, num_elements)
                self.fp32_partitioned_groups_flat_new[i].data = OPTIMIZER_STATES_CACHE.get(buffer_id, num_elements)
                self.fp32_partitioned_groups_flat_new[i].final_location = OffloadDeviceEnum.cpu
                if param.ds_tensor.status == PartitionedParamStatus.NOT_AVAILABLE:
                    self._swap_in_sub_group_to_flat_buffer(self.fp32_partitioned_groups_flat_new[i], i)
                else:
                    self.fp32_partitioned_groups_flat_new[i].data.copy_(param.ds_tensor.data.float())   #I will allocate it later during tensor distribution in different storage 
                self.fp32_partitioned_groups_flat_new[i].grad = OPTIMIZER_STATES_CACHE.get_gradient_buffer(buffer_id, num_elements)
            else:
                self.fp32_partitioned_groups_flat_new[i].final_location = OffloadDeviceEnum.nvme
                self.fp32_param_location[i] = OffloadDeviceEnum.nvme
                if param.ds_tensor.status == PartitionedParamStatus.NOT_AVAILABLE:
                    unpinned_fp32_buffer = torch.empty(num_elements, device='cpu', dtype=torch.float32)
                    self._swap_in_sub_group_to_flat_buffer(unpinned_fp32_buffer, i)
                    self.optimizer_swapper.initialize_parameters(parameters=[self.fp32_partitioned_groups_flat_new[i]],
    ...
    def move_params16_grad_to_cpu(self):

        self.reduce_bucket_size = gpu_grad_threshold * self.largest_param_numel
    ...
    def _configure_tensor_swapping(self, offload_optimizer_config, aio_config):
        ...
        if not prefetchtable.get_warmup():
            self.optimizer_swapper = swapper_type(swap_config=offload_optimizer_config,
                                                ...
                                                base_folder=nvme_swap_folder,
                                                optimizer=self.optimizer,
                                                largest_numel=self.largest_param_numel,
                                                device=self.device,
                                                dtype=torch.float32,
                                                timers=self.timers)
    ...
    def _create_fp16_partitions_with_defragmentation(self, fp16_param_groups):
        ...
        for param_group_idx, param_group in enumerate(param_groups):
            for sub_group in param_group:
                ...
                total_param_count += len(sub_group)
        ...
        #if not self.offload_param or prefetchtable.get_warmup():  # partitioned params remain in GPU during training
    ...
    def _swap_in_sub_group_to_flat_buffer(self, flat_buffer, sub_group_id):
        ...
        if prefetchtable.get_warmup():
            ...
            param = self.param_dict[self.fp32_partitioned_groups_flat_new[sub_group_id].ds_id]
            if param.ds_tensor.status == PartitionedParamStatus.NOT_AVAILABLE:
                print_rank_0(
                    f"Swapping in {param.ds_id} with elements {param.ds_numel} and partition {param.partition_numel()}"
                )
                param.nvme_swapper.swap_in([param], async_op=False)
                flat_buffer.data.copy_(param.ds_tensor.data.float())
                param.nvme_swapper.remove_partition_and_release_buffers([param])
                print_rank_0(f"Swapping in {param.ds_id} done")
    ...
    def _create_fp32_partitions(self):
        ...
        if self.offload_optimizer:
            ...
            for i in range(sub_group_size):
                #if i < int(self.partial_offload * sub_group_size) and not prefetchtable.get_warmup():
    ...
    def _optimizer_step(self, sub_group_id):
        if prefetchtable.get_warmup():
            ...
            fp32_param = self.fp32_partitioned_groups_flat_new[sub_group_id]
            param_group_id = self.sub_group_to_group_id_new[fp32_param.ds_id]
            if self.offload_optimizer:
                self.optimizer.param_groups[param_group_id]['params'] = [fp32_param]
                cpu_loss = self.optimizer.step()
                self.optimizer.param_groups[param_group_id]['params'] = []
    ...
    def _partitioned_params_swap_out(self, i):
        ...
        if prefetchtable.get_warmup():
            ...
            swap_fp16_params.append(param_16)
            if len(swap_fp16_params):
                swap_fp16_params[0].nvme_swapper.swap_out_partitioned_params(dst_fp16_params=swap_fp16_params,
                                                                            src_fp32_params=swap_fp32_params)
    ...
    def reduce_independent_p_g_buckets_and_remove_grads(self, param):
        ...
        #if len(self.params_in_ipg_bucket) + 1 > gpu_grad_threshold and self.elements_in_ipg_bucket > 0:
    ...
    def partition_grads(self, params_to_release: List[Parameter], grad_partitions: List[Tensor]) -> None:
        ...
        for param, grad_partition in zip(params_to_release, grad_partitions):
            ...
            if prefetchtable.get_warmup():
                ...
                if hasattr(self.inf_or_nan_tracker, "logical_or_"):
                    self.inf_or_nan_tracker.logical_or_(torch.isinf(grad_partition).any())
                    self.inf_or_nan_tracker.logical_or_(torch.isnan(grad_partition).any())
                else:
                    # logical_or_ not available in older versions of pytorch
                    self.inf_or_nan_tracker += torch.isinf(grad_partition).any()
                    self.inf_or_nan_tracker += torch.isnan(grad_partition).any()
                ...
                if self.is_gradient_accumulation_boundary:
                    ...
                    if self._swappable_optimizer_subgroup(i):
                        ...
                        if prefetchtable.get_warmup():
                            ...
                            offload_fp32_gradients[i].append(grad_partition.float())
                            offload_fp32_offsets[i].append(dest_offset)
                    else:
                        if prefetchtable.get_warmup():
        ...
        if self.offload_optimizer and self.swap_optimizer:
            if prefetchtable.get_warmup():
                ...
                for i in offload_fp32_gradients.keys():
                    self.optimizer_swapper.swap_out_gradients(parameter=self.fp32_partitioned_groups_flat_new[i],
                                                            gradient_offsets=offload_fp32_offsets[i],
                                                            gradient_tensors=offload_fp32_gradients[i])
    ...
    def _release_sub_group(self, sub_group_id, timer_names):
        ...
        if prefetchtable.get_warmup():
            ...
            if prefetchtable.FP32_IN_NVME:
                ...
                if self.fp32_partitioned_groups_flat_new[sub_group_id].final_location == OffloadDeviceEnum.nvme:
                    self.do_fetch_fp32_nvme = True

                if self.do_fetch_fp32_nvme:
                    buffer_id = OPTIMIZER_STATES_CACHE.get_param_id_to_buffer_id(self.fp32_partitioned_groups_flat_new[sub_group_id].ds_id)
                    OPTIMIZER_STATES_CACHE.release_buffer_id(self.fp32_partitioned_groups_flat_new[sub_group_id].ds_id, buffer_id, self.fp32_partitioned_groups_flat_new[sub_group_id].ds_numel)
                    self.fp32_param_location[sub_group_id] = OffloadDeviceEnum.nvme
                    self._optimizer_states_and_gradient_swap_out(sub_group_id, timer_names)
                    ...
                    if self.fp32_current_row == len(self.fp32_partitioned_groups_flat_new):
                        self.do_fetch_fp32_nvme = False
                    self.fp32_current_row = self.fp32_current_row % len(self.fp32_partitioned_groups_flat_new)
    ...
    def _optimizer_states_and_gradient_swap_out(self, sub_group_id, timer_names):
        if prefetchtable.get_warmup():
            ...
            # get rid of the fp32 gradients. Not needed anymore
            self.fp32_partitioned_groups_flat_new[sub_group_id].grad = None
    ...
    def _unflatten_partitioned_parameters(self, sub_group_id):
        ...
        for partitioned_param, q, param_id in zip(self.fp16_partitioned_groups[sub_group_id], updated_params, self.fp16_partitioned_groups_flat_id[sub_group_id]):
    ...
    def _reassign_or_swap_out_partitioned_parameters(self, sub_group_id):
        ...
        if prefetchtable.get_warmup():
            ...
            param_16 = self.param_dict[self.fp32_partitioned_groups_flat_new[sub_group_id].ds_id]
            index = prefetchtable.get_tensor_id_to_prefetch_list_id(param_16.ds_id, "fwd")
            current_location = prefetchtable.get_prefetch_information_list_item(index).current_location
            if current_location == OffloadDeviceEnum.nvme:
                self._partitioned_params_swap_out(sub_group_id)
            else:
                param_16.ds_tensor.data.copy_(self.fp32_partitioned_groups_flat_new[sub_group_id].data)
    ...
    def step(self, closure=None):
        ...
        if prefetchtable.get_warmup():
            ...
            #for sub_group_id, group in enumerate(self.fp16_groups):
            for sub_group_id, (key, value) in enumerate(self.param_dict.items()):
        ...
        #optim_state = self.backup_optimizer.state
    ...
    def unscale_and_clip_grads(self, sub_group_id, total_norm):
        ...
        if not prefetchtable.get_warmup():
            self.fp32_partitioned_groups_flat_new[sub_group_id].grad.mul_(1. / combined_scale)
    ...
    def has_overflow(self, partition_gradients=True):
        ...
        if partition_gradients:
            with get_accelerator().stream(self.reduce_and_partition_stream):
                if prefetchtable.get_warmup():
                    if hasattr(self.inf_or_nan_tracker, "logical_or_"):
                        # logical_or_ available in pytorch
                    ...
                    if not hasattr(self.inf_or_nan_tracker, "logical_or_"):
                        self.inf_or_nan_tracker = self.inf_or_nan_tracker > 0
