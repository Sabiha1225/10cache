...
class PrefetchTable:
    ...
    class PrefetchInformation:
        def __init__(self, tensor_id, tensor_type, layer_id, fwd, tensor_element=0, start_time=0.0, end_time=0.0, activation_duration=0.0) -> None:
        ...
        def set_activation_duration(self):
            self.activation_duration = self.end_time - self.start_time

        def __repr__(self):
            return (
                f"Tensor ID: {self.tensor_id}\n"
                f"Tensor Type: {self.tensor_type}\n"
                f"Layer ID: {self.layer_id}\n"
                f"FWD: {self.fwd}\n"
                f"tensor_element: {self.tensor_element}\n"
                f"Activation Duration: {self.activation_duration}\n"
            )
    ...
    def __new__(cls, *args, **kwargs):
        if cls._instance is None:
            cls._instance = super(PrefetchTable, cls).__new__(cls)
    ...
    def __init__(self) -> None:
        self.prefetch_information_list = []
        self.tensor_id_to_prefetch_list_id = {}
        self.param_id_to_module_id = {}
        self.warmup = True
        self.current_row = 0
        self.eval = False
        self.PARAMS_IN_NVME = False
        self.FP32_IN_NVME = False

    def add_tensor_information(self, tensor_id, tensor_type, layer_id, fwd, tensor_element=0):
        self.prefetch_information_list.append(self.PrefetchInformation(tensor_id, tensor_type, layer_id, fwd, tensor_element))
    
    def get_prefetch_information_list(self):
        return self.prefetch_information_list
    
    def get_prefetch_information_list_item(self, index):
        return self.prefetch_information_list[index]
    
    def get_prefetch_information_list_len(self):
        return len(self.prefetch_information_list)
    
    def add_tensor_id_to_prefetch_list_id(self, tensor_id, fwd, index):
        if tensor_id not in self.tensor_id_to_prefetch_list_id:
            self.tensor_id_to_prefetch_list_id[tensor_id] = {}
        self.tensor_id_to_prefetch_list_id[tensor_id][fwd] = index
    
    def get_tensor_id_to_prefetch_list_id(self, tensor_id, fwd):
        return self.tensor_id_to_prefetch_list_id[tensor_id][fwd]
    
    def get_tensor_id_to_prefetch_list_id_dict(self):
        return self.tensor_id_to_prefetch_list_id
    
    def add_param_id_to_module_id(self, param_id, module_id):
        self.param_id_to_module_id[param_id] = module_id
    
    def get_param_id_to_module_id(self, param_id):
        return self.param_id_to_module_id[param_id]
    
    def get_param_id_to_module_id_dict(self):
        return self.param_id_to_module_id
...
class MemoryManager:
    ...
    def __init__(self) -> None:
        ...
        self.gpu_allocated_memory = None
        self.gpu_max_allocated_memory = None
        self.gpu_available_memory = None
        self.cpu_total_memory = None
        self.cpu_used_memory = None
        self.cpu_available_memory = None

    def update_memory_status(self):

        # self.gpu_total_memory = round(get_accelerator().total_memory() / (1024 * 1024 * 1024), 2 )
        # self.gpu_allocated_memory = round(get_accelerator().memory_allocated() / (1024 * 1024 * 1024), 2 )
        # self.gpu_max_allocated_memory = round(get_accelerator().max_memory_allocated() / (1024 * 1024 * 1024), 2 )
        # self.gpu_available_memory = self.gpu_total_memory - self.gpu_allocated_memory
        # self.cpu_total_memory = round(psutil.virtual_memory().total / (1024 * 1024 * 1024), 2 )
        # self.cpu_available_memory = round(psutil.virtual_memory().available / (1024 * 1024 * 1024), 2 )
        # self.cpu_used_memory = self.cpu_total_memory - self.cpu_available_memory

        # in bytes
        self.gpu_total_memory = round(get_accelerator().total_memory(), 2 )
        self.gpu_allocated_memory = round(get_accelerator().memory_allocated(), 2 )
        self.gpu_max_allocated_memory = round(get_accelerator().max_memory_allocated(), 2 )
        self.gpu_available_memory = self.gpu_total_memory - self.gpu_allocated_memory
        self.cpu_total_memory = round(psutil.virtual_memory().total, 2 )
        self.cpu_available_memory = round(psutil.virtual_memory().available, 2 )
        self.cpu_used_memory = self.cpu_total_memory - self.cpu_available_memory

    def get_memory_status(self):
        self.update_memory_status()
        memory_stat = {}
        memory_stat['gpu_total_memory'] = self.gpu_total_memory
        memory_stat['gpu_allocated_memory'] = self.gpu_allocated_memory
        memory_stat['gpu_max_allocated_memory'] = self.gpu_max_allocated_memory
        memory_stat['gpu_available_memory'] = self.gpu_available_memory
        memory_stat['cpu_total_memory'] = self.cpu_total_memory
        memory_stat['cpu_available_memory'] = self.cpu_available_memory
        memory_stat['cpu_used_memory'] = self.cpu_used_memory
...
class MemoryBufferAllocation:
    # _instance = None

    # def __new__(cls, *args, **kwargs):
    #     if cls._instance is None:
    #         cls._instance = super(MemoryBufferAllocation, cls).__new__(cls)
    #     return cls._instance
    
    def __init__(self):
        self.param_id_to_buffer_id = {}

    def create_memory_buffer(self, num_buffers, numel, dtype, device):
        self.num_buffers = num_buffers
        self.buffers = [
            self.allocate_mem_buff(numel, dtype, device)
            for i in range(num_buffers)]
        self._index = -1
        self.free_buffer_ids = [i for i in range(self.num_buffers)]

    def add_param_id_to_buffer_id(self, param_id, buffer_id):
        self.param_id_to_buffer_id[param_id] = buffer_id

    def get_param_id_to_buffer_id(self, param_id):
        return self.param_id_to_buffer_id[param_id]
    
    def get_free_buffer_id(self):
        assert len(self.free_buffer_ids) > 0, f"No buffer available to swap param."
        buffer_id = self.free_buffer_ids.pop()
        return buffer_id
    
    def release_buffer_id(self, param_id, buffer_id):
        self.free_buffer_ids.append(buffer_id)
        del self.param_id_to_buffer_id[param_id]

    def allocate_mem_buff(self, numel, dtype, device):
        memorybuffer = self.MemoryBuffer(numel, dtype, device)
        return memorybuffer
    ...
    class MemoryBuffer:
        def __init__(self, numel, dtype, device):
            ...
            if device == 'cpu':
                self.data = get_accelerator().pin_memory(torch.empty(self.numel, dtype=self.dtype, device=self.device))
            else:
                self.data = torch.empty(self.numel, dtype=self.dtype, device=self.device)
        ...
        def add(self, tensor):
            ...
            tensor_numel = tensor.ds_numel
            #tensor_numel = tensor.numel()
            new_end = self._start + tensor_numel
            assert new_end <= self.numel, \
                'Not enough memory left in the buffer ({} > {})'.format(
                    tensor_numel, self.numel - self._start)
            # New tensor is a view into the memory.
            #new_tensor = self.data[self._start:new_end]
            new_tensor = self.data.narrow(0, self._start, tensor_numel)
            self._end = new_end
            #new_tensor = new_tensor.view(tensor.shape)
            new_tensor.copy_(tensor.data)
        ...
        def get_data(self):
            ...
            return self.data[self._start:self._end]

class MemoryBufferAllocationClassified:
    # _instance = None
    ...
    def add_free_bufferes(self, buffer_class, buffer_size, buffer_class_count, dtype, device):
        if buffer_class not in self.free_buffer_ids:
            self.free_buffer_ids[buffer_class] = []
        old_num_buffers =  self.num_buffers
        self.num_buffers += buffer_class_count
        for id in range(old_num_buffers, self.num_buffers):
            self.free_buffer_ids[buffer_class].append(id)
            self.buffers.append(self.allocate_mem_buff(buffer_size, dtype, device))
    ...
    def get_free_buffer_id(self, buffer_class):
        assert len(self.free_buffer_ids[buffer_class]) > 0, f"No buffer available for size {buffer_class} to swap param."
        buffer_id = self.free_buffer_ids[buffer_class].pop()
        return buffer_id
    
    def release_buffer_id(self, param_id, buffer_id, buffer_class):
        self.free_buffer_ids[buffer_class].append(buffer_id)
        del self.param_id_to_buffer_id[param_id]

    def allocate_mem_buff(self, numel, dtype, device):
        memorybuffer = self.MemoryBufferClassified(numel, dtype, device)
        return memorybuffer
    
    class MemoryBufferClassified:
        def __init__(self, numel, dtype, device):
            # if torch.distributed.get_rank() == 0:
            #     element_size = torch.tensor([], dtype=dtype).element_size()
            #     print('> building the {} memory buffer with {} num elements '
...
class MemoryBufferAllocationClassifiedContiguous:
    # _instance = None
    ...
    def __init__(self):
        ...
        self.buffers = []
        self.free_buffer_ids = {}
        self.occupied_buffer_ids = {}
        self.gpu_tensors_cpu_cache_ids = {}
        self.num_buffers = 0
    ...
    def add_free_bufferes(self, buffer_size_count_mapping, dtype, device):
        total_buffer_size = 0
        #fp16_element_size = torch.tensor([], dtype=torch.float16).element_size()
        element_size = torch.tensor([], dtype=dtype).element_size()
        for tensor_size in buffer_size_count_mapping:
            count = buffer_size_count_mapping[tensor_size]
            total_buffer_size += (int(tensor_size/element_size) * count)

        if device == 'cpu':
            self.data = get_accelerator().pin_memory(torch.empty(total_buffer_size, dtype=dtype, device=device))
        else:
            self.data = torch.empty(total_buffer_size, dtype=dtype, device=device)

        offset = 0
        for tensor_size in buffer_size_count_mapping:
            count = buffer_size_count_mapping[tensor_size]
            tensor_element = int(tensor_size/element_size)
            if tensor_element not in self.free_buffer_ids:
                self.free_buffer_ids[tensor_element] = []

            old_num_buffers =  self.num_buffers
            self.num_buffers += count
            for id in range(old_num_buffers, self.num_buffers):
                self.free_buffer_ids[tensor_element].append(id)
                self.buffers.append(self.data.narrow(0, offset, tensor_element))
                offset += int(tensor_element)


    def add(self, tensor, buffer_id):
        """Allocate a chunk of memory from the buffer to tensor and copy
        the values."""
        # Number of elements of the input tensor.
        #tensor_numel = torch.numel(tensor)
        ...
        # New tensor is a view into the memory.
        #new_tensor = self.data[self._start:new_end]
        new_tensor = self.buffers[buffer_id].narrow(0, 0, tensor_numel)
        #self._end = new_end
        #new_tensor = new_tensor.view(tensor.shape)
        #new_tensor.copy_(tensor.data, non_blocking=True)
        new_tensor.copy_(tensor.data)
        # Return a pointer to the new tensor.
        return new_tensor

    def get(self, buffer_id, tensor_numel):
        """Allocate a chunk of memory from the buffer to tensor and copy
        the values."""
        new_tensor = self.buffers[buffer_id].narrow(0, 0, tensor_numel)
        return new_tensor
    ...
    def get_free_buffer_id(self, buffer_class):
        if len(self.free_buffer_ids[buffer_class]) == 0:
            with open("/home/sabiha/deepspeed_example/deepspeed_gpu_cpu_tensor_count.txt", 'a') as file:
                file.write(f"/deepspeed/runtime/zero/stage3.py#LN479 inside def get_free_buffer_id buffer_class {buffer_class} self.occupied_buffer_ids[buffer_class] {self.occupied_buffer_ids[buffer_class]} self.free_buffer_ids {self.free_buffer_ids} \n")
        assert len(self.free_buffer_ids[buffer_class]) > 0, f"No buffer available for size {buffer_class} to swap param."
        buffer_id = self.free_buffer_ids[buffer_class].pop()
        return buffer_id

    def has_free_buffer_id(self, buffer_class):
        if len(self.free_buffer_ids[buffer_class]) > 0:
            return True
        else:
            return False
    ...
    def release_buffer_id(self, param_id, buffer_id, buffer_class):
        self.free_buffer_ids[buffer_class].append(buffer_id)
        self.occupied_buffer_ids[buffer_class].remove(param_id)
        del self.param_id_to_buffer_id[param_id]

    def add_occupied_buffer_id(self, param_id, buffer_class):
        if buffer_class not in self.occupied_buffer_ids:
            self.occupied_buffer_ids[buffer_class] = []
        self.occupied_buffer_ids[buffer_class].append(param_id)

    def get_occupied_buffer_id(self, buffer_class):
        assert len(self.occupied_buffer_ids[buffer_class]) > 0, f"No occupied buffer available for size {buffer_class} to swap param."
        param_id = self.occupied_buffer_ids[buffer_class][0]
        return param_id

    def add_gpu_tensors_cpu_cache_id(self, buffer_class, param_id):
        if buffer_class not in self.gpu_tensors_cpu_cache_ids:
            self.gpu_tensors_cpu_cache_ids[buffer_class] = []
        self.gpu_tensors_cpu_cache_ids[buffer_class].append(param_id)

    def has_gpu_tensors_cpu_cache_id(self, buffer_class):
        if len(self.gpu_tensors_cpu_cache_ids[buffer_class]) > 0:
            return True
        else:
            return False

    def get_gpu_tensors_cpu_cache_id(self, buffer_class):
        assert len(self.gpu_tensors_cpu_cache_ids[buffer_class]) > 0, f"No buffer available for size {buffer_class} to swap param."
        param_id = self.gpu_tensors_cpu_cache_ids[buffer_class].pop()
        return param_id

    def release_gpu_tensors_cpu_cache_id(self, param_id, buffer_class):
        if buffer_class in self.gpu_tensors_cpu_cache_ids:
            if param_id in self.gpu_tensors_cpu_cache_ids[buffer_class]:
                self.gpu_tensors_cpu_cache_ids[buffer_class].remove(param_id)
...
class MemoryBufferAllocationOptimizerStates:
    # _instance = None
    ...
    def add_free_bufferes(self, buffer_size_count_mapping, dtype, device):
        ...
        element_size = torch.tensor([], dtype=dtype).element_size()
        for tensor_size in buffer_size_count_mapping:
            count = buffer_size_count_mapping[tensor_size] * 4
            total_buffer_size += (int(tensor_size/element_size) * count)
        ...
        for tensor_size in buffer_size_count_mapping:
            ...
            for id in range(old_num_buffers, self.num_buffers):
                ...
                self.buffers.append(self.data.narrow(0, offset, tensor_element))
                offset += int(tensor_element)
                self.buffers_grad.append(self.data.narrow(0, offset, tensor_element))
                offset += int(tensor_element)
                self.optimizer_momentum.append(self.data.narrow(0, offset, tensor_element))
                offset += int(tensor_element)
                self.optimizer_variance.append(self.data.narrow(0, offset, tensor_element))
                offset += int(tensor_element)
    ...
    def get_gradient_buffer(self, buffer_id, tensor_numel):
        """Allocate a chunk of memory from the buffer to tensor and copy
        the values."""
        new_tensor = self.buffers_grad[buffer_id].narrow(0, 0, tensor_numel)
        return new_tensor

    def get_momentum_buffer(self, buffer_id, tensor_numel):
        """Allocate a chunk of memory from the buffer to tensor and copy
        the values."""
        new_tensor = self.optimizer_momentum[buffer_id].narrow(0, 0, tensor_numel)
        return new_tensor
    
    def get_variance_buffer(self, buffer_id, tensor_numel):
        """Allocate a chunk of memory from the buffer to tensor and copy
        the values."""
        new_tensor = self.optimizer_variance[buffer_id].narrow(0, 0, tensor_numel)
        return new_tensor
...
class OffloadCountInfo:

    class OffloadCount:
        def __init__(self, tensor_id, gpu_fetch_count, gpu_offload_count) -> None:
            self.tensor_id = tensor_id
            self.gpu_fetch_count = gpu_fetch_count
            self.gpu_offload_count = gpu_offload_count

        def __repr__(self):
            return (
                f"Tensor ID: {self.tensor_id}\n"
                f"gpu_fetch_count: {self.gpu_fetch_count}\n"
                f"gpu_offload_count: {self.gpu_offload_count}\n"
            )
    ...
    # def __new__(cls, *args, **kwargs):
    #     if cls._instance is None:
    #         cls._instance = super(OffloadCountInfo, cls).__new__(cls)
    #     return cls._instance
    ...
    def __init__(self) -> None:
        self.offload_count_dict = {}
        self.gpu_read_bytes = 0
        self.gpu_write_bytes = 0

    def add_OffloadCount_information(self, tensor_id, gpu_fetch_count, gpu_offload_count):
        self.offload_count_dict[tensor_id] = self.OffloadCount(tensor_id, gpu_fetch_count, gpu_offload_count)
    
    def get_OffloadCount_dict(self):
        return self.offload_count_dict
    
    def get_OffloadCount_dict_item(self, tensor_id):
        return self.offload_count_dict[tensor_id]



prefetchtable = PrefetchTable()
memory_manager = MemoryManager()
# GPU_MEMORY_BUFFER = MemoryBufferAllocation()
# CPU_MEMORY_BUFFER = MemoryBufferAllocation()
GPU_MEMORY_CACHE = MemoryBufferAllocationClassifiedContiguous()
CPU_MEMORY_CACHE = MemoryBufferAllocationClassifiedContiguous()
OPTIMIZER_STATES_CACHE = MemoryBufferAllocationOptimizerStates()

active_tensor_window = []
params_id_list = {}
gpu_threshold = 5
cpu_threshold = 0
fp32_threshold = 0
gpu_grad_threshold = 5
GPU_TENSOR_SIZE_COUNT = {}
CPU_TENSOR_SIZE_COUNT = {}
OPTIMIZER_STATES_TENSOR_SIZE_COUNT = {}
additional_buffer_factor = 2.5
cpu_additional_buffer_factor = 2
# optimizer_states_buffer_factor = .67
optimizer_states_buffer_factor = .35
swap_space_gpu = 3
swap_space_cpu = 5
nvme_buffer_space = 6
min_buffer_count = 20
...
CPU_RANK = 2
NVME_RANK = 4
offloadcountinfo = OffloadCountInfo()