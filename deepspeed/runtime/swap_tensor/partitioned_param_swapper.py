...
class AsyncPartitionedParameterSwapper(object):
    ...
    def clear_buffer(self):
        ...
        del self.buffers
        del self.available_buffer_ids

    def allocate_buffer(self, buffer_count, buffer_size):
        self.elements_per_buffer = buffer_size
        self.aligned_elements_per_buffer = self._io_aligned_numel(self.elements_per_buffer)
        self.param_buffer_count = buffer_count

        self.available_buffer_ids = [i for i in range(self.param_buffer_count)]
        self.buffers = get_accelerator().pin_memory(torch.empty(int(self.aligned_elements_per_buffer * self.param_buffer_count), dtype=self.dtype, requires_grad=False), align_bytes=0)
    ...
    def _allocate_and_return_buffers_for_swap_in(self, params):
        ...
        for param in params:
            param_id = param.ds_id
            # if param_id not in self.param_id_to_numel.keys():
            #     self.param_id_to_numel[param_id] = param.ds_tensor.ds_numel
    ...
    def release_buffers(self, params):
        for param in params:
            param_id = param.ds_id
    ...
    def remove_partition(self, params):
        for param in params:
            param_id = param.ds_id
            if param_id in self.available_params:
                self.available_params.remove(param_id)
    ...
    #blocking swap out followed by releasing the memory buffers
    def swap_out_and_release_from_buffer(self, param, src_buffer, async_op=False, force_buffer_release=False):
        if async_op:
            assert force_buffer_release, "Should not release preallocated buffers without completing the swap out. Set force_buffer_release to True to do it anyways"
        
        swap_out_paths = self._get_swap_paths(param)
        swap_out_params = [src_buffer]
        self._track_numel(param)
