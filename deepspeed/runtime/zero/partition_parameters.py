...
from deepspeed.runtime.utils import prefetchtable, memory_manager, GPU_MEMORY_CACHE, CPU_MEMORY_CACHE, params_id_list, offloadcountinfo
    ...
    def __init__(self, param: Parameter) -> None:
        ...
        if hasattr(param.ds_tensor, "ds_quant_scale"):
            ...
            if not prefetchtable.get_warmup():
                ...
                if prefetchtable.get_prefetch_information_list_item(index).current_location == OffloadDeviceEnum.nvme:
                    param.ds_tensor.data = GPU_MEMORY_CACHE.add(param.ds_tensor, gpu_buffer_id)
                    if not get_accelerator().is_synchronized_device():
                        get_accelerator().current_stream().synchronize()

                    param.nvme_swapper.release_buffers([param])
                else:
    ...
    def _partition_param(self, param, buffer=None, has_been_updated=False):
        ...
        if param.ds_status is ZeroParamStatus.AVAILABLE:
            ...
            if param.ds_tensor is not None and not has_been_updated:  ##param already partitioned
                ...
                if param.ds_tensor.final_location == OffloadDeviceEnum.nvme and prefetchtable.get_warmup():
                ...
                if not prefetchtable.get_warmup():
                    if prefetchtable.PARAMS_IN_NVME:
                        index = prefetchtable.get_tensor_id_to_prefetch_list_id(param.ds_id, "fwd")
                        if prefetchtable.get_prefetch_information_list_item(index).final_location == OffloadDeviceEnum.nvme and param.ds_tensor.final_location == OffloadDeviceEnum.nvme:
                            tensor_element = param.ds_tensor.ds_numel
                            gpu_buffer_id = GPU_MEMORY_CACHE.get_param_id_to_buffer_id(param.ds_id)
                            GPU_MEMORY_CACHE.release_buffer_id(param.ds_id, gpu_buffer_id, tensor_element)
                            ...
                            if CPU_MEMORY_CACHE.has_free_buffer_id(tensor_element):
                                ...
                                if CPU_MEMORY_CACHE.has_gpu_tensors_cpu_cache_id(tensor_element):
                                    gpu_param_id = CPU_MEMORY_CACHE.get_gpu_tensors_cpu_cache_id(tensor_element)
                                    gpu_param = params_id_list[gpu_param_id]
                                    gpu_param_buffer_id = CPU_MEMORY_CACHE.get_param_id_to_buffer_id(gpu_param_id)
                                    index1 = prefetchtable.get_tensor_id_to_prefetch_list_id(gpu_param_id, "fwd")
                                    prefetchtable.get_prefetch_information_list_item(index1).current_location = OffloadDeviceEnum.nvme
                                    index1 = prefetchtable.get_tensor_id_to_prefetch_list_id(gpu_param_id, "bwd")
                                    prefetchtable.get_prefetch_information_list_item(index1).current_location = OffloadDeviceEnum.nvme
                                    buffer = gpu_param.nvme_swapper.get_buffer(gpu_param, gpu_param.ds_tensor.ds_numel)
                                    ...
                                    occupied_buffer_id = CPU_MEMORY_CACHE.get_param_id_to_buffer_id(occupied_param_id)
                                    index1 = prefetchtable.get_tensor_id_to_prefetch_list_id(occupied_param_id, "fwd")
                                    prefetchtable.get_prefetch_information_list_item(index1).current_location = OffloadDeviceEnum.nvme
                                    index1 = prefetchtable.get_tensor_id_to_prefetch_list_id(occupied_param_id, "bwd")
                                    prefetchtable.get_prefetch_information_list_item(index1).current_location = OffloadDeviceEnum.nvme
                                    buffer = occupied_param.nvme_swapper.get_buffer(occupied_param, occupied_param.ds_tensor.ds_numel)
                            ...
                            GPU_MEMORY_CACHE.release_buffer_id(param.ds_id, gpu_buffer_id, tensor_element)
                            index1 = prefetchtable.get_tensor_id_to_prefetch_list_id(param.ds_id, "fwd")
