...
from transformers import AutoModelForSequenceClassification, AdamW, get_scheduler, AutoModelForCausalLM
import csv
import pandas as pd
...
checkpoint = "tiiuae/Falcon3-10B-Base"
...
train_dataloader = DataLoader(
    lm_datasets["train"], shuffle=True, batch_size=16, collate_fn=data_collator
)
eval_dataloader = DataLoader(
    lm_datasets["validation"], batch_size=16, collate_fn=data_collator
)
...
def main(args):
    ...
    for epoch in range(warm_up_epoch):  # loop over the dataset multiple times
        ...
        for i, data in enumerate(train_dataloader):
            ...
            labels = data["labels"] # For causal language modeling
    ...
    with open("/home/sabiha/deepspeed_example/training_time.txt", 'a') as file:
        file.write(f"Training Time taken smart-cache with 10B Falcon model: {training_time / args.epochs} s \n")
